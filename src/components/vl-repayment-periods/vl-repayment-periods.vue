...
/* Start section for repayment periods */
const getData = ref(null)

async function fetchData () {
  try {
    getData.value = await fetchAPI('/requested-repayment-periods')
  } catch (err) {
...
<template>
  <div class="sm:col-span-3">
    ...
    >{{ lang.vlRepaymentPeriods }}</label>
    <div class="mt-2">
      <select
        v-model="modelValue"
        name="repayment-periods"
        autocomplete="repayment-periods"
